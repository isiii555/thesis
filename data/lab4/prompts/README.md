...
| `prompt1\_zero\_shot.md` | Contains a zero-shot prompt: one new log input X is provided and the AI agent must produce y without any prior example. |

| `prompt2\_few\_shot.md` | Contains a few-shot prompt: two previous (X,y) examples are provided before generating y for a new input. |
